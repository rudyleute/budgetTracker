...
import {loginFormUtils} from '../resolvers/loginResolver';
import {useForm} from 'react-hook-form';
import {useMemo} from 'react';
import {faGoogle} from '@fortawesome/free-brands-svg-icons';
import LinkIcon from '../components/simple/LinkIcon';
import {LoginArg} from "../types/accountProvider";
...
const LoginPage = () => {
    ...
    const {
        ...
        defaultValues: {
            email: "",
            password: ""
        } satisfies Record<keyof LoginArg, unknown>,
        mode: "onSubmit",
    ...
    return (
        <div onSubmit={handleSubmit((values) => logIn(values as LoginArg))}
             className={"w-full h-screen bg-(--color-sec) relative"}>
            <form className={"w-[350px] max-esml:w-[95%] flex flex-col window-center gap-[15px] relative bg-(--color-main) rounded-[15px] p-[20px_20px_35px_20px]"}>
                <div className={"grid grid-cols-[2fr_4fr_2fr] gap-[5px] items-center"}>
                    <div className={"w-fit"}>
                        <LinkIcon to={"/signup"} className={"mr-1"} title={"Sign up"}
                                  iClassName={"text-(--color-third)"} icon={faUserCircle}/>
