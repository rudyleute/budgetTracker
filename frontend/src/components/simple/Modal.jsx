...
const Modal = ({ children, onClose, onSubmit, bClassName, title, zIndex }) => {
  return (createPortal(
    <div
      className={"font-bold text-[var(--color-text)] window-center w-full max-w-[480px] box-content h-[550px] rounded-[15px]"}
      style={{ zIndex }}>
      ...
      <div className={twMerge("bg-[var(--color-main)] p-[15px_25px] s-scroll s-scroll-comp overflow-x-hidden overflow-y-auto min-h-full h-full", bClassName)}>
        {children}
      </div>
    </div>, document.body))
}
