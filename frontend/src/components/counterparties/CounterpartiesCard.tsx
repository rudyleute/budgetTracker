...
import LinkIcon from '../simple/LinkIcon';
...
const CounterpartiesCard = ({counterparty}: CounterpartiesCardProps) => {
    ...
    return (
        ...
        <span className={"w-full grid grid-cols-3"}>
          ...
          <FontAwesomeIcon className={"justify-self-center"} size={"3x"} color={"var(--color-third)"}
                           icon={faCircleUser}/>
          <IconCell className={"justify-self-end"}>
            <IconButton title={"Delete counterparty"}
                        ...
                        onClick={
                            () => showConfirmation({
                                onAccept: () => deleteCounterparty(counterparty.id),
