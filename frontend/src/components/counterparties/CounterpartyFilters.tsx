...
import Select, {SelectOption} from '../simple/Select.jsx';
...
import {onFormSubmit, SubmitWithoutId} from '../../helpers/utils.js';
...
const CounterpartyFilters = ({counterparty}: CounterpartyFiltersProps) => {
    ...
    const priorityOptions: SelectOption[] = useMemo(
        () => priorities.filter(priority => priority !== loansQueryParams.priority).map((priority, ind) => ({
            label: priority,
            id: String(ind)
        })),
    ...
    const onLoanCreate = useCallback(
        async () => onFormSubmit<PagEntityGet<'loan'>>(
            ...
            hideModal), [addLoan, hideModal]
    );

    const handleCreation = useCallback(() => {
        showModal({
            title: "New loan",
            content: <LoansForm onSubmit={onLoanCreate} ref={formRef} counterparty={counterparty}/>,
