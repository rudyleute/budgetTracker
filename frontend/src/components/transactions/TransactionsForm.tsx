import {forwardRef, ReactNode, useCallback, useEffect, useMemo, useRef} from 'react';
import Input from '../simple/Input';
import Select from '../simple/Select';
import {useCategories} from '../../context/CategoriesProvider';
import {useModal} from '../../context/ModalProvider';
import Color from '../simple/Color';
import {faFolderPlus} from '@fortawesome/free-solid-svg-icons';
import IconButton from '../simple/IconButton';
import CategoriesForm from '../categories/CategoriesForm';
import {faPenToSquare, faXmarkCircle} from '@fortawesome/free-regular-svg-icons';
import {useConfirmation} from '../../context/ConfirmationProvider';
import {getDatetimeLocal} from '../../helpers/time';
import {useForm} from 'react-hook-form';
import {transactionFormUtils, TransactionSchemaType} from '../../resolvers/transactionResolver';
import Button from '../simple/Button';
import {onFormSubmit, SubmitWithId, SubmitWithoutId, validateFields} from '../../helpers/utils';
import {FormRef, UpdatableFormProps} from "../../types/basic";
import {useFormRef} from "../../hooks/useFormRef";
import {CategoryGetClient, TransactionGetClient} from "../../types/components/mappings";

type TransactionsFormProps = UpdatableFormProps & (
    | { isUpdate: true } & { transaction: TransactionSchemaType }
    | { isUpdate?: false }
);
const TransactionsForm = forwardRef<FormRef, TransactionsFormProps>((props, ref) => {
    const {onSubmit, isUpdate} = props;
    const {name, categoryId, timestamp, price} = isUpdate ? props.transaction : {};

    const {resolver: transactionResolver, fieldsMeta} = useMemo(() => {
        return transactionFormUtils();
    }, []);
    const {categories, addCategory, editCategory, deleteCategory} = useCategories();
    const {data: catData, dataMap: catDataMap} = categories;
    const {showConfirmation} = useConfirmation();
    const {showModal, hideModal} = useModal();
    const catFormRef = useRef<FormRef>(null);

    const form = useForm({
        resolver: transactionResolver,
        defaultValues: {
            name: name || "",
            categoryId: categoryId || "",
            price: price || "",
            timestamp: ""
        } satisfies Record<keyof TransactionSchemaType, unknown>,
        mode: "onSubmit",
        reValidateMode: "onSubmit"
    });

    const {
        register,
        trigger,
        getValues,
        formState: {errors},
        setValue,
        clearErrors,
        formState,
        watch
    } = form;

    useFormRef(ref, form);

    const fields = watch();
    const updateCategory = useCallback((id: string) => setValue("categoryId", id, {
        shouldDirty: true, shouldValidate: true
    }), [setValue]);

    useEffect(() => {
        //The timestamp should be dirtied up before when creating a new transaction since the default value is generated
        if (isUpdate) setValue("timestamp", getDatetimeLocal(new Date(timestamp!)));
        else setValue("timestamp", getDatetimeLocal(new Date(Date.now())), {shouldDirty: true});
    }, [setValue, isUpdate, timestamp]);

    const onCategoryEdit = useCallback(
        async (id: string) => onFormSubmit<CategoryGetClient>(
            catFormRef.current?.getData,
            editCategory as SubmitWithId<CategoryGetClient>,
            async (category) => {
                if (category.id === fields.categoryId) updateCategory(category.id); //if the updated category was the selected one
                hideModal();
            }, id
        ),
        [editCategory, fields.categoryId, hideModal, updateCategory])

    const onCategoryCreate = useCallback(
        async () => onFormSubmit(
            catFormRef.current?.getData,
            addCategory as SubmitWithoutId<CategoryGetClient>,
            async (category) => {
                updateCategory(category.id);
                hideModal();
            }),
        [addCategory, hideModal, updateCategory]
    )


    const formLabel = ({id, name, color}: {
        id: CategoryGetClient["id"],
        name: TransactionGetClient["name"],
        color: CategoryGetClient["color"]
    }): ReactNode => {
        return <span className={"flex items-center justify-between gap-2.5"}>
        <span className={"text-clipped"}>
          <Color value={color}/>
          <span className={"ml-2.5"}>{name}</span>
        </span>
        <span>
          <IconButton title={"Edit category"} size={"xs"} icon={faPenToSquare} onClick={() => {
              showModal({
                  title: "Edit category",
                  content: <CategoriesForm onSubmit={() => onCategoryEdit(id)} ref={catFormRef} category={{
                      name, color
                  }} isUpdate={true}/>,
                  saveFunc: () => onCategoryEdit(id),
                  hideOnSave: false
              })
          }}/>
          <IconButton title={"Delete category"} size={"xs"} icon={faXmarkCircle} onClick={() => {
              showConfirmation({
                  onAccept: async () => {
                      if (await deleteCategory(id) && id === fields.categoryId) updateCategory("")
                  },
                  text: `'${name}' category`
              })
          }}/>
        </span>
      </span>
    }

    const options = catData.filter(cat => cat.id !== fields.categoryId).map(item => ({
        label: formLabel(item),
        id: item.id
    }))

    const handleCatCreate = useCallback(() => {
        showModal({
            title: "Add category",
            content: <CategoriesForm onSubmit={onCategoryCreate} ref={catFormRef}/>,
            saveFunc: onCategoryCreate,
            hideOnSave: false
        })
    }, [onCategoryCreate, showModal]);

    return (
        <form onSubmit={async (e) => {
            e.preventDefault();
            if (onSubmit) {
                const data = await validateFields(trigger, getValues(), formState.dirtyFields);

                if (data) onSubmit(data);
            }
        }} className={"grid max-modal:grid-cols-1 modal:grid-cols-[2fr_1fr] gap-2.5"}>
            <Input required={fieldsMeta.name.required} wClassName={"col-span-full modal:row-start-1"} label={"Name"}
                   id={"name"} type={"text"} {...register("name", {
                onChange: () => clearErrors("name")
            })} error={errors.name}
            />
            <Input required={fieldsMeta.timestamp.required} label={"Timestamp"} id={"timestamp"}
                   type={"datetime-local"} {...register("timestamp", {
                onChange: () => clearErrors("timestamp")
            })} error={errors.timestamp}
            />
            <Input required={fieldsMeta.price.required} label={"Price"} id={"price"} type={"number"} min={0}
                   step={0.01} {...register("price", {
                onChange: () => clearErrors("price")
            })} error={errors.price}
            />

            <Select
                className={"col-span-full"}
                value={fields.categoryId ? formLabel(catDataMap[fields.categoryId]) : ""}
                lClassName={"flex items-center"}
                required={fieldsMeta.categoryId.required}
                label={
                    <>
                        <span className={"mr-[3px]"}>Category</span>
                        <IconButton
                            onClick={handleCatCreate}
                            className={"leading-0"}
                            iconClassName={"!text-(--color-text)"}
                            title={"Add category"}
                            icon={faFolderPlus}
                        />
                    </>
                }
                onOptionClick={({id}) => updateCategory(id)}
                options={options}
                error={errors.categoryId}
            />
            <Button type={"submit"} className={"hidden"} aria-hidden={"true"} tabIndex={-1}/>
        </form>
    );
});

export default TransactionsForm;