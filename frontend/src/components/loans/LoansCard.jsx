...
const LoansCard = ({ loan, onAfterEdit, onAfterDeleteSuccess }) => {
  ...
  return (
    <div className={"w-full h-fit hover:cursor-pointer sml:lift-scale"} title={"Edit loan"} onClick={handleOnEdit}>
      <div
        className={twMerge('relative w-full h-full items-center font-bold grid grid-cols-[1fr_10fr_1fr] animate-fade-in text-(--color-text) bg-(--color-main) rounded-[30px] p-[20px_10px] max-cards-sml:p-[30px_30px]', `${loan.isDue && 'due'}`)}>
        ...
        </IconCell>
        {
          loan.deadline ? (() => {
            ...
            return (
              <span className={twMerge("font-b text-clipped h-full justify-self-center text-(--color-pos)!", days <= 0 && "text-(--color-third)!")}
        ...
        <span className={twMerge("price-wrapper col-span-full p-[0_10px] bg-(--color-third)/80!", loan.type === "lent" && "bg-(--color-pos)/80!")}>{loan.sum} €</span>
      </div>
    </div>
  )
}
